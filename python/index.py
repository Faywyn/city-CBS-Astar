...
This script visualizes vehicle traffic data from CSV files generated by the city-CBS-Astar simulation.

CSV Format
----------
The input CSV file should have the following format:
    numCar;density;speed1;speed2;...;speedN

Where:
    - numCar: Number of vehicles in the simulation (integer)
    - density: Traffic density value (float)
    - speed1, speed2, ..., speedN: Individual vehicle speeds in m/s (float)

Output
------
The script generates a plot with the following elements:
    - Blue vertical bars: Individual data points (speeds) for each numCar value
    - Red line: Mean speed for each numCar value
    - Green line: Trend line (polynomial interpolation) for the mean speed
    - Purple dashed lines: Standard deviation bands (±1 standard deviation)
    - X-axis labels: "numCar (density)" format
...
Example:
    python index.py data/traffic_results.csv

Configuration
-------------
All plotting parameters (colors, line styles, etc.) can be customized in the
...
def main():
    """
    Main function to process CSV data and generate visualization.
    
    Reads a CSV file from command line argument, processes the data,
    computes statistics, and generates a comprehensive plot showing
    vehicle speeds vs. number of vehicles with trend analysis.
    """

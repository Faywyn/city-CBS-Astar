...
constexpr double SIM_STEP_TIME = 0.1;   // in seconds
constexpr int CBS_PRECISION_FACTOR = 1; // CBS_PRECISION_FACTOR * SIM_STEP_TIME must not be to high
constexpr double CBS_MAX_SUB_TIME = 5;  // in seconds

// For hash functions (to reduce items that are really close to each other)
constexpr double CELL_SIZE = 1.0;                 // in meters
constexpr double SPEED_RESOLUTION = 1.3;          // in m/s
constexpr double ANGLE_RESOLUTION = 0.3;          // in radians
